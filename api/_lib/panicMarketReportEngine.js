/**
 * 서버 전용 — vite-project ESM/React 의존 없음
 */
import { computeMarketAction } from "./panicMarketActionEngine.js"
import { interpretPanicMetric } from "./panicMetricInterpretLite.js"
...
function buildSummary(snaps, action) {
  ...
  const parts = []
  if (byKey.vix) parts.push(`VIX ${byKey.vix.valueText}로 ${statusWord(byKey.vix.statusLabel, byKey.vix.tone)}`)
  if (byKey.highYield) {
    parts.push(
  ...
  switch (action.regime) {
    case "extreme_fear":
      line2 = "단기 변동성·헤지 수요가 크므로 추격 매수보다 방어와 유동성 확보가 우선입니다."
      break
    case "fear":
      line2 = "단기 과열 신호는 일부 존재할 수 있으나, 시스템 리스크는 아직 통제 가능한 공포 구간으로 해석됩니다."
      break
    case "greed":
      line2 = "위험 선호 심리가 살아 있으나, 일부 지표에서 과열·쏠림이 동시에 관측됩니다. 분할 접근이 유효합니다."
      break
    case "extreme_greed":
      line2 = "탐욕·과열 신호가 우세합니다. 추격보다 익절·비중 조절로 변동성 확대에 대비하세요."
      break
    default:
      line2 = "단기 과열과 방어 신호가 혼재합니다. 이벤트 대응형·범위 매매가 유효한 중립 환경입니다."
  }
  return `${line1}\n\n${line2}`
}
