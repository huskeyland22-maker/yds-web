...
assert.ok(Array.isArray(briefing.reasons))
assert.ok(Array.isArray(briefing.narrative))
assert.ok(briefing.narrative.length <= 3)
assert.ok(!briefing.reasons.some((r) => /최종 해석|분할매수를 시작|공격적으로 매수/.test(r)))

const conclusion = buildTodayMarketConclusion(panicData, historyRows, null, cycleFlow)
assert.ok(conclusion.visible)
assert.ok(conclusion.headline)
