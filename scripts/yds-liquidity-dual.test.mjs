...
assert.equal(report.market.title, "유동성 환경")
assert.equal(report.policy.title, "정책 유동성")
assert.ok(report.policy.environment.some((f) => /PCE/.test(f.label)))
...
assert.ok(report.policy.scoreExplain?.includes("정책 유동성"))
assert.ok(report.market.scoreExplain?.includes("유동성"))
