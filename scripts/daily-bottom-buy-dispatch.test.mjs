...
describe("daily-bottom-buy hobby dispatch", () => {
  ...
  it("vercel rewrite maps /api/daily-bottom-buy → market-data?ydsMode=daily-bottom-buy", () => {
    ...
    const tradeHit = vercel.rewrites?.find((r) => r.source === "/api/trade-records-sync")
    assert.ok(tradeHit, "missing trade-records-sync rewrite")
    assert.equal(tradeHit.destination, "/api/portfolio-sync?ydsMode=trade-records")
