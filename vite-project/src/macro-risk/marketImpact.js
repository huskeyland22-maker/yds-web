...
export function buildMarketImpact(raw, pillars, triggers) {
  ...
  const rateRepricingEvent = triggers.some((t) => t.id === "rate_repricing_event" && t.active)
  ...
  const growthPressure = rateScore >= 58 || rateShock || rateRepricingEvent || riskAsset
  ...
  return [
    row(
      ...
      rateRepricingEvent ? "금리 재평가 이벤트" : growthPressure ? "금리·변동성 압박" : "균형",
    ),
    row(
      "semi",
      "반도체",
      easing ? "favorable" : growthPressure || riskAsset ? "risk" : "neutral",
      rateRepricingEvent ? "AI/반도체 주의" : "베타·금리 민감",
    ),
    row(
      "kospi",
      "코스피",
      liqScore >= 60 && dxyUp
        ? "risk"
        : kospiRateRiskBoost || rateShock || rateRepricingEvent
          ? "risk"
