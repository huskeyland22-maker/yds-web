...
export default function TacticalZoneAuxPanel({ position, activeAux, expandedAux, onToggle }) {
  ...
  return (
    <div className="tactical-zone-aux-panel">
      ...
      <div
        className={["tactical-zone-aux-panel__collapse", isOpen ? "tactical-zone-aux-panel__collapse--open" : ""].join(
          " ",
        )}
        aria-hidden={!isOpen}
      >
        {detail ? (
          <div
            className="tactical-zone-aux-detail"
            data-tone={detail.statusTone}
            role="region"
            aria-label={`${detail.title} 상세`}
          >
