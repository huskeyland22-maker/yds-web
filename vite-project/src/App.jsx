...
import AiPerformanceDashboardPage from "./pages/AiPerformanceDashboardPage.jsx"
...
function App() {
  ...
  const inputReady = useMemo(() => {
    try {
      return REQUIRED_KEYS.every((key) => coerceMetricValue(parsedData?.[key]) != null)
    } catch {
      return false
    }
  }, [parsedData])
      ...
      <div className="app-column-host flex min-w-0 flex-1 flex-col overflow-visible">
        ...
        <main className="app-main-host flex-1 overflow-visible px-2.5 py-2 pb-[calc(3.75rem+env(safe-area-inset-bottom))] sm:px-4 lg:px-6 lg:py-5 lg:pb-5">
          <Routes>
            ...
            <Route
              path="/performance-validation/dashboard"
              element={
                <SectionErrorBoundary label="AI 성과 대시보드">
            ...
            <div className="mt-1.5 flex shrink-0 items-center justify-between gap-2 font-mono text-[10px] text-slate-500">
              ...
              {inputText.trim() && missingRequired.length > 0 ? (
                <span className="text-amber-200/80">
                  누락: {missingRequired.map((k) => FIELD_LABELS[k] ?? k).join(", ")}
                </span>
              ) : inputText.trim() ? (
