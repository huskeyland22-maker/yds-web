...
const STAGE_ONE_LINER = {
  overheated: "공포 없음 — 현금 확보와 추격 자제가 우선입니다.",
  neutral: "공포 부족 — 종목 탐색·선별 관찰 단계입니다.",
  interest: "관심 — 우량주 추적과 현금 비중 유지가 핵심입니다.",
  dca: "분할매수 — 준비된 현금을 나눠 투입할 시기입니다.",
  panicBuy: "인생 타점 — 계획된 현금 투입을 검토할 수 있습니다.",
}

/** @type {Record<string, string>} */
const STAGE_EXPLANATION = {
  overheated:
    "패닉 강도가 낮아 매수 기회가 제한적입니다. 주식 비중을 낮추고 현금을 확보하며, 신규 추격 매수는 제한하는 것이 권장됩니다.",
  neutral:
    "아직 인생 타점 단계는 아닌 공포 부족 구간입니다. 시장은 관찰·종목 선별에 적합하며, 무리한 일괄 매수보다 탐색이 우선입니다.",
  interest:
  ...
  panicBuy:
    "역사적 공포에 가까운 인생 타점 구간입니다. 사전에 준비한 현금을 계획대로 투입할 수 있는 국면으로 해석됩니다.",
}
...
const ACTION_GUIDE_BY_STAGE = {
  ...
  panicBuy: "오늘은 계획된 현금을 활용해 인생 타점 후보를 분할 집행하세요.",
}
...
export function buildAiDailyReportFromMarketAnalysis(report) {
  ...
  const bullLine =
    bullSim != null && bullSim >= 55 && stageId !== "panicBuy"
      ? "강세장 유사도는 높지만 아직 인생 타점 단계는 아닙니다."
      : bullSim != null && stageId === "panicBuy"
        ? "강세장 유사도와 함께 인생 타점 국면으로 해석됩니다."
        : bullSim != null
