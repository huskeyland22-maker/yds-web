 ...
 * - milestone 순서: start → rise → fearExpansion → climax → recovery (날짜 엄격 증가)
 */

/** @typedef {import("./ydsHistoricalEventTypes.js").YdsEventCategory} YdsEventCategory */
...
function assertAllMilestoneOrders() {
  for (const event of RAW_EVENTS) {
    const check = validateMilestoneOrder(event.keyDates)
    if (!check.valid) {
      console.warn(`[YDS events] milestone order invalid: ${event.id}`, check.violations)
    }
  }
}

if (import.meta.env?.DEV) {
  assertAllMilestoneOrders()
}

export { YDS_MILESTONE_ORDER }

export const YDS_VALIDATION_EVENT_DATASET = RAW_EVENTS.map((event) => ({
  ...event,
  phase: event.category,
  event: event.name,
  milestones: buildMilestonesFromKeyDates(event.keyDates),
